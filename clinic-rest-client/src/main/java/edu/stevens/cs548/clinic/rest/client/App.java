...
public class App {
	...
	private SurgeryTreatmentDto addSurgeryTreatment() throws IOException, ParseException {
		...
		msg("Patient ID: ");
		surgery.setPatientId(UUID.fromString(in.readLine()));
		msg("Provider ID: ");
		surgery.setProviderId(UUID.fromString(in.readLine()));
		msg("Diagnosis: ");
		surgery.setDiagnosis(in.readLine());
		surgery.setSurgeryDate(readDate("Date of Surgery"));
		msg("Discharge Instructions: ");
		surgery.setDischargeInstructions(in.readLine());

// 		msg("Follow Up Treatments? ([Y]es / [N]o): ");
//		if (in.readLine().equals("Y")) {
//			List<TreatmentDto> followUpTreatments = addTreatmentList();
//			List<UUID> treatmentUuids = new ArrayList<>();
//			for (TreatmentDto followUpTreatment : followUpTreatments) {
//				// adding created treatments here to persist
//				treatments.add(followUpTreatment);
//				treatmentUuids.add(followUpTreatment.getId());
//			}
//			surgery.setFollowUpTreatments(treatmentUuids);
//		}
	...
	private RadiologyTreatmentDto addRadiologyTreatment() throws IOException, ParseException {
		...
		msg("Patient ID: ");
		radiology.setPatientId(UUID.fromString(in.readLine()));
		msg("Provider ID: ");
		radiology.setProviderId(UUID.fromString(in.readLine()));
		msg("Diagnosis: ");
		radiology.setDiagnosis(in.readLine());
		msg("Treatment Dates: (enter a comma separated list of dates)");
		String dateInput = in.readLine(); // 02/02/1993, 02/12/1992
		String[] dateStrings = dateInput.split(",");
		List<LocalDate> dates = new ArrayList<>();
		for (String dateString : dateStrings) {
			dates.add(LocalDate.parse(dateString.trim(), dateFormatter));
		}
		radiology.setTreatmentDates(dates);

		return radiology;
	}

	private PhysiotherapyTreatmentDto addPhysiotherapyTreatment() throws IOException {
		PhysiotherapyTreatmentDto physiology = treatmentFactory.createPhysiotherapyTreatmentDto();

		physiology.setId(UUID.randomUUID());
		msg("Patient ID: ");
		physiology.setPatientId(UUID.fromString(in.readLine()));
		msg("Provider ID: ");
		physiology.setProviderId(UUID.fromString(in.readLine()));
		msg("Diagnosis: ");
		physiology.setDiagnosis(in.readLine());
		...
		physiology.setTreatmentDates(dates);
	...
	public void upload(String[] inputs) throws IOException {
		...
		IStreamingOutput output = new IStreamingOutput() {
			...
			public void write(OutputStream output) throws IOException {
				...
				try (JsonWriter wr = gson
						.newJsonWriter(new BufferedWriter(new OutputStreamWriter(output)))) {
