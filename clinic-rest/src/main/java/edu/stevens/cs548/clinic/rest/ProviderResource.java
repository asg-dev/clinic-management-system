...
public class ProviderResource extends ResourceBase {
	...
	public Response getProvider(@PathParam("id") String id) {
		try {
			 ...
			 */
			for (TreatmentDto treatmentDto : provider.getTreatments()) {
				responseBuilder.link(getTreatmentUri(uriInfo, treatmentDto.getProviderId(), treatmentDto.getId()), TREATMENT);
			}
